...
function connect() {
  for (let a = 0; a < particlesArray.length; a++) {
    for (let b = a; b < particlesArray.length; b++) {
      ...
      if (dist2 < (canvas.width/7) * (canvas.height/7)) {
...
/* ============================
   ...
   - la imagen "sube" al scrollear
   - el canvas se "esconde" un poco en el hero
   ============================ */
(() => {
  ...
  const onScroll = () => {
    ...
    // progreso mientras el hero está “pasando”
    const total = Math.max(1, heroH - vh);
    const scrolled = clamp(-heroRect.top, 0, total);
    const p = scrolled / total;

    // ✅ Parallax: la imagen sube suavemente
    const y = -Math.round(p * 80);     // sube hasta 80px
    const s = 1.08 + (p * 0.02);       // micro zoom
    heroImg.style.transform = `translateY(${y}px) scale(${s})`;

    // ✅ Canvas: más discreto en el hero, vuelve luego
    const start = 0.10; // opacidad en hero
    const end = 0.55;   // opacidad normal
    const t = clamp((p - 0.20) / 0.80, 0, 1);
...
(() => {
  ...
  const buildDots = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      d.addEventListener("click", () => { index = i; update(true); restart(); });
  ...
  const update = (snap = false) => {
    ...
    if (maxIndex === 0) {
      ...
      dotsWrap && (dotsWrap.style.display = "");
    }
  ...
  const next = () => { index = (index >= getMaxIndex()) ? 0 : index + 1; update(); };
  const prev = () => { index = (index <= 0) ? getMaxIndex() : index - 1; update(); };

  prevBtn?.addEventListener("click", () => { prev(); restart(); });
  nextBtn?.addEventListener("click", () => { next(); restart(); });

  const start = () => { if (!autoplayEnabled) return; stop(); timer = setInterval(next, INTERVAL); };
  const stop = () => { if (timer) clearInterval(timer); timer = null; };
  const restart = () => { stop(); start(); };

  root.addEventListener("mouseenter", () => { autoplayEnabled = false; stop(); });
  root.addEventListener("mouseleave", () => { autoplayEnabled = true; start(); });

  const rebuild = () => { dots = buildDots(); update(true); restart(); };
  window.addEventListener("resize", () => rebuild());

  requestAnimationFrame(() => { dots = buildDots(); update(true); start(); });
})();
